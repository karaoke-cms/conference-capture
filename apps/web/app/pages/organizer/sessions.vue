...
const bySession = computed(() => contributionGroups(dashboard.value?.contributions ?? []));
const countsFor = (sessionId: string) => {
  const items = bySession.value.get(sessionId) ?? [];
  return { contributions: items.length, photos: items.filter((item) => item.mediaUrl).length };
};
